...
int main()
    ...
    if (NULL == CU_add_test(pSuite, "Unarmoured, no passphrase", test_rsa_decrypt_noarmour_nopassphrase))
	{
	CU_cleanup_registry();
	return CU_get_error();
	}
    
    if (NULL == CU_add_test(pSuite, "Armoured, no passphrase", test_rsa_decrypt_armour_nopassphrase))
	{
	CU_cleanup_registry();
	return CU_get_error();
	}
    
    if (NULL == CU_add_test(pSuite, "Unarmoured, passphrase", test_rsa_decrypt_noarmour_passphrase))
	{
	CU_cleanup_registry();
	return CU_get_error();
	}
    
    if (NULL == CU_add_test(pSuite, "Armoured, passphrase", test_rsa_decrypt_armour_passphrase))
	{
	CU_cleanup_registry();
	return CU_get_error();
	}

...
void ops_sign_file(const char* filename, const ops_secret_key_t *skey)
    ...
    /*    ops_memory_t* mem_ld=NULL;
    ops_create_info_t* cinfo_ld=NULL;
    */
    ...
    mem_buf=ops_write_buf_from_file(filename);
    ...
    // set up signature
    sig=ops_create_signature_new();
    ops_signature_start_signature(sig,(ops_secret_key_t *)skey, hash_alg, sig_type);
    // set up output file

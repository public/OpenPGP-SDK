...
static void
init_subregion (ops_region_t * subregion, ops_region_t * region)
{
...
static void
format_error (ops_parser_content_t * content, const char *const fmt, ...)
{
  va_list va;
...
static ops_reader_ret_t
base_read (unsigned char *dest, unsigned *plength,
	   ops_reader_flags_t flags, ops_parse_options_t * opt)
{
  ops_reader_ret_t ret = opt->reader (dest, plength, flags, opt->reader_arg);
  if (ret != OPS_R_OK && !(ret == OPS_R_EOF && !(flags & OPS_RETURN_LENGTH)))
    return ret;
 ...
 */
static ops_reader_ret_t
read_scalar (unsigned *result, unsigned length, ops_parse_options_t * opt)
{
  unsigned t = 0;
 ...
 */
int
ops_limited_read (unsigned char *dest, unsigned length,
		  ops_region_t * region, ops_parse_options_t * opt)
{
  ...
  if (base_read
      (dest, &length, region->indeterminate ? OPS_RETURN_LENGTH : 0,
       opt) != OPS_R_OK)
    ERR ("Read failed");
 ...
 */
static int
parse_user_id (ops_region_t * region, ops_parse_options_t * opt)
{
  ops_parser_content_t content;
 ...
 */
static int
parse_v3_signature (ops_region_t * region, ops_parse_options_t * opt)
{
  unsigned char c[1];
  ops_parser_content_t content;
			       ...
			       ops_parse_options_t * opt)
{
  ...
  switch (content.tag)
    ...
    case OPS_PTAG_SS_PREFERRED_SKA:
      /* should this be malloc-ed? or done as fixed array?
       * use fixed arraysize for now. Ben? - Rachel */
 ...
 */
static int
parse_v4_signature (ops_region_t * region, ops_parse_options_t * opt,
 ...
 */
static int
parse_signature (ops_region_t * region, ops_parse_options_t * opt)
{
  unsigned char c[1];
  ops_parser_content_t content;
  size_t v4_hashed_data_start;
...
static int
parse_compressed (ops_region_t * region, ops_parse_options_t * opt)
{
  unsigned char c[1];
  ops_parser_content_t content;
 ...
 */
static int
ops_parse_one_packet (ops_parse_options_t * opt)
{
  char ptag[1];
  ops_reader_ret_t ret;
  ops_parser_content_t content;
 ...
 */
int
ops_parse (ops_parse_options_t * opt)
{
  int r;
...
void
ops_parse_options (ops_parse_options_t * opt,
		   ops_content_tag_t tag, ops_parse_type_t type)
{
  int t8, t7;
  ...
  if (tag == OPS_PTAG_SS_ALL)
    {
      ...
      for (n = 0; n < 256; ++n)
	ops_parse_options (opt, OPS_PTAG_SIGNATURE_SUBPACKET_BASE + n, type);
      return;
    }
